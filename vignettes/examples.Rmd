...
cat(export_interactive_roc(myrocplot, cutoffs = rocdata$c, font.size = "12px", prefix = "a", color = "red"))
```
...
```{r inter, eval = FALSE}
plot_interactive_roc(myrocplot, cutoffs = rocdata$c)
```
...
cat(export_interactive_roc(myrocplot, cutoffs = rocdata$c, font.size = "12px", prefix = "aci", color = "red", lwd = 1))
```
...
plot_journal_roc(rocplot, datalist, lty = rep(1, 3), color = c("blue", "green", "purple"), legend = TRUE)
```

```{r test-multi, fig.keep='none', results = 'asis', echo = TRUE, fig.width=6, fig.height=6}
cat(export_interactive_roc(rocplot, cutoffs = lapply(datalist, function(d) d$c), 
