...
StatPrc <- ggproto(`_class` = "StatPrc",
                   `_inherit` = Stat,
                   ...
                   compute_group = function(data, scales, na.rm = TRUE, max.num.points = 1e3, increasing = TRUE){
                     ...
                     m_order <- order(data$m, decreasing = increasing) ## this is confusing but think about it for a sec
                     
                     m_sorted <- data$m[m_order]
                     TP <- cumsum(outcome[m_order] == 1)
                     FP <- cumsum(outcome[m_order] == 0)

                     ## remove fp & tp for duplicated predictions
                     ## Highest cutoff (Infinity) corresponds to tp=0, fp=0
                     
                     dups <- rev(duplicated(rev(m_sorted)))
                     TP <- TP[!dups]
                     FP <- FP[!dups]
                     m_sorted <- m_sorted[!dups]
                     
                     if (!is.null(max.num.points)) {
                       idxs <- seq(from = 1, to = length(m_sorted), length.out = max.num.points)
                       TP <- TP[idxs]
                       FP <- FP[idxs]
                       m_sorted <- m_sorted[idxs]
                     }
                     
                     recall <- c(0, TP)/sum(outcome == 1)
                     prec <- c(0, TP/(TP + FP))
                     
                     Lowest <- ifelse(test = increasing, yes = Inf, no = -Inf)
                     cutoffs <- c(Lowest, m_sorted)
                     
                     data.frame(precision = prec, recall = recall, cutoffs = cutoffs)
                     
                   })

#' Calculate the empirical Precision Recall curve
#' 
#' Given a binary outcome d and continuous measurement m, computes the empirical
#' PR curve for assessing the classification accuracy of m
#' 
...
#' \describe{
#'   \item{precision}{estimate of precision = TP / (TP + FP)}
#'   \item{recall}{estimate of recall = TP / (TP + FN)}
...
#' Empirical Precision Curve
#' 
#' Display the empirical PR curve. Useful for characterizing the classification
...
# #'   displaying rectangular confidence regions for the empirical ROC curve, \code{\link{style_prc}} for
